...
const Navbar = () => {
  ...
  const fetchSublinks = async () => {
    try {
      ...
      console.log(
        "Something went wrong in fetching category list from backend",
        err
      );
      console.error(err);
    }
  ...
  console.log("Sublinks", subLinks.length);
  console.log("Sublinks is ", subLinks);
    ...
    <div className="border-b border-richblack-700 bg-richblack-800  ">
      <div className="flex w-11/12 max-w-maxContent items-center justify-between mx-auto py-2 ">
                  ...
                  <li key={index}>
                    {element.title === "Catalog" ? (
                      <div className="relative cursor-pointer group ">
                        ...
                        {/* hover */}
                        <div
                          className="invisible absolute flex flex-col rounded-lg bg-richblack-5 z-10
                        ...
                        <p
                          className={`${
                            tab === element.title
                              ? "text-yellow-50 "
                              : " text-richblack-25"
                          } font-normal text-base py-1 px-3 `}
        ...
        <div className=" flex flex-row gap-5 items-center ">
          ...
          {token === null && (
            <Link to={"/login"}>
              <button className="rounded-lg border py-2 px-3 border-richblack-700 bg-richblack-800 font-medium text-base text-richblack-100 ">
                Log in
              </button>
            </Link>
          )}
